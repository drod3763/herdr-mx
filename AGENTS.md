...
- `src/protocol/wire.rs::PROTOCOL_VERSION` gates the exact-match client/server handshake, so a bump strands every other-versioned peer (including upstream) until matched. Bump it ONLY for a breaking change to the binary `ClientMessage`/`ServerMessage` wire format — NOT for additive JSON API methods. A new method (e.g. a new `Method`/`ResponseResult` variant) is backward-compatible: old clients never send it, and a server that lacks it returns a normal method error the caller degrades on, so it needs no bump. When a breaking wire-format change does require a bump, compare `PROTOCOL_VERSION` against the latest released tag and bump only if the current source protocol is not already greater than the latest released protocol, then update hardcoded protocol expectations and manual protocol fixtures in tests.
