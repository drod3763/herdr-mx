...
impl HeadlessServer {
    ...
    pub(super) fn render_retained_graphics_update_and_stream(&mut self) -> RetainedGraphicsOutcome {
        ...
        for (_, terminal_size, _, _, mode, surface_mode) in &render_targets {
            if !matches!(mode, ClientConnectionMode::App) {
                continue;
            }
            // mx embedded-content surfaces render a pane subset with their own
            // geometry, so the retained pane-graphics fast path cannot serve them.
            if !matches!(surface_mode, crate::protocol::ClientSurfaceMode::FullApp) {
                crate::render_prof::event("retained_graphics_fallback.embedded_content_client");
                return RetainedGraphicsOutcome::Fallback;
            }
        ...
        for (client_id, (cols, rows), cell_size, _is_foreground, mode, _surface_mode) in
            render_targets
        {
            if !matches!(mode, ClientConnectionMode::App) {
                continue;
