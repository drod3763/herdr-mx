...
mod tests {
    ...
    fn late_full_lifecycle_hook_with_same_session_after_process_exit_does_not_reacquire_authority()
    {
        ...
        terminal.set_detected_state(Some(Agent::Pi), AgentState::Working);
        // Pin every event to an explicit clock: the wrapper's internal Instant::now()
        // can land before `now + 1ms` on a fast runner, making the "late" hook not
        // late at all and flaking this test.
        terminal.set_hook_authority_at(
            "herdr:pi".into(),
            "pi".into(),
            AgentState::Working,
