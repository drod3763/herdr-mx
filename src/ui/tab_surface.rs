...
mod tests {
    ...
    async fn desktop_full_app_semantic_frame_is_characterized() {
        ...
        // herdr-mx: this digest characterizes the MX desktop frame — the mx sidebar renders
        // its own segment layout, so upstream's digest for the same state does not apply.
        assert_eq!(
            frame_digest(&frame),
