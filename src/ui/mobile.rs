...
/// Workspace rows for the mobile switcher, in grouped (worktree-tree) order.
/// herdr-mx's sidebar entry list can also carry `Divider`/`HostBanner` rows, which the
/// mobile switcher does not render — filter to workspaces so row math stays consistent.
fn mobile_workspace_entries(app: &AppState) -> Vec<(usize, bool)> {
...
fn render_mobile_switcher_content(
    ...
    for (entry_idx, (ws_idx, indented)) in space_entries.iter().enumerate() {
        ...
        let detail_prefix = if *indented {
            let last_child = !matches!(space_entries.get(entry_idx + 1), Some((_, true)));
