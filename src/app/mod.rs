...
pub struct App {
    ...
    /// #11: single-flight guard for `remote.ssh_config_hosts`. `Some(started)` while a discovery
    /// worker is running; each request caps concurrency to one so a flood of local API calls cannot
    /// pile up threads/IO. Timestamped so a stale in-flight (a worker hung on a bad/network-mounted
    /// `~/.ssh`) is recoverable after a lease instead of poisoning discovery until server restart.
    pub(crate) ssh_discovery_in_flight: Arc<std::sync::Mutex<Option<Instant>>>,
...
impl App {
    pub fn new(
        ...
        let mut state = AppState {
            ...
            sidebar_section_split,
            agent_panel_sort,
            sidebar_agents: config.ui.sidebar.agents.row_layout.clone(),
            sidebar_spaces: config.ui.sidebar.spaces.row_layout.clone(),
        ...
        Self {
            ...
            render_notify,
            render_dirty,
            ssh_discovery_in_flight: Arc::new(std::sync::Mutex::new(None)),
    ...
    fn apply_live_config(
        ...
        if !invalid_section("ui") {
            ...
            if let Some(diagnostic) = config.invalid_sidebar_bounds_diagnostic() {
                ...
                self.state.sidebar_agents = config.ui.sidebar.agents.row_layout.clone();
                self.state.sidebar_spaces = config.ui.sidebar.spaces.row_layout.clone();
            ...
            lines: vec![
                ...
                vec![
                    ...
                    crate::config::SidebarItem {
                        field: crate::config::SidebarAgentField::SpaceName,
                        show: false,
