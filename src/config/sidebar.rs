...
impl SidebarTokenColor {
    // herdr-mx: only tests read the parsed color; the mx segment renderer replaced the
    // upstream token-row renderer that consumed it. Kept so the accepted config stays typed.
    #[allow(dead_code)]
...
impl SpaceSidebarToken {
    // herdr-mx: only tests split token/style; the mx segment renderer replaced the upstream
    // token-row renderer that consumed it. Kept so the accepted config stays typed.
    #[allow(dead_code)]
    pub(crate) fn parts(&self) -> (&Self, SidebarTokenStyle) {
        match self {
            Self::Styled { token, style } => (token, *style),
...
mod tests {
    ...
    fn defaults_match_the_compact_agent_and_existing_space_layouts() {
        let agents = AgentsSidebarConfig::default();
        let spaces = SpacesSidebarConfig::default();
        assert_eq!(
            agents.rows,
            vec![
                vec![
        ...
        assert!(agents.rows_by_agent.is_empty());
        assert_eq!(agents.row_gap, 0);
        assert_eq!(
            spaces.rows,
            vec![
                vec![SpaceSidebarToken::StateIcon, SpaceSidebarToken::Workspace],
...
"#,
        ...
        assert_eq!(
            config.ui.sidebar.agents.row_layout.rows[1],
            vec![
        ...
        assert_eq!(
            config.ui.sidebar.agents.row_layout.rows[2],
            vec![
        ...
        assert_eq!(
            config.ui.sidebar.agents.row_layout.rows_by_agent["claude"],
            vec![
        ...
        assert_eq!(config.ui.sidebar.agents.row_layout.row_gap, 1);
        assert_eq!(
            config.ui.sidebar.spaces.row_layout.rows[1],
            vec![SpaceSidebarToken::Custom("jj_status".into())]
        );
        assert_eq!(config.ui.sidebar.spaces.row_layout.row_gap, 3);
    }
...
"##,
        ...
        let (token, style) = config.ui.sidebar.agents.row_layout.rows[0][0].parts();
        ...
        assert_eq!(
            config.ui.sidebar.agents.row_layout.rows[0][1],
            AgentSidebarToken::Workspace
        );

        let (token, style) =
            config.ui.sidebar.agents.row_layout.rows_by_agent["claude"][0][0].parts();
        assert_eq!(token, &AgentSidebarToken::Agent);
        assert_eq!(style.bold, Some(true));
        assert_eq!(style.dim, Some(false));

        let (token, style) = config.ui.sidebar.spaces.row_layout.rows[0][0].parts();
        assert_eq!(token, &SpaceSidebarToken::GitStatus);
        assert_eq!(
            style.fg.unwrap().ratatui(),
            ratatui::style::Color::Rgb(0xff, 0x00, 0xaa)
        );
        let (token, style) = config.ui.sidebar.spaces.row_layout.rows[1][0].parts();
    ...
    fn accepts_every_canonical_agent_override_key() {
        ...
        assert_eq!(
            config.ui.sidebar.agents.row_layout.rows_by_agent.len(),
            agents.len()
        );
    }
