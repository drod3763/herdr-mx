...
When enabled, `herdr --remote` writes a private temporary SSH config that includes your `~/.ssh/config` and `/etc/ssh/ssh_config` first, then adds fallback `ServerAliveInterval` and `ServerAliveCountMax` values. Your own SSH keepalive settings win. Set `manage_ssh_config = false` to run the bridge through plain `ssh` without Herdr's generated config. herdr-mx does not use upstream's per-attach OpenSSH control socket for connection reuse; for reconnecting transports, see `[remote.transport]` below.

To reach remotes with a program other than `ssh` (for example `autossh` or a wrapper script), set `[remote.transport]`:
...
Herdr spawns `program` with `args`, expanding `{host}` and `{remote_command}` within a token and the standalone token `{options}` to each resolved SSH option as its own argument. A custom transport owns its full argv (no `-T`/`ConnectTimeout` is injected) and must provide a raw bidirectional binary stdio channel; terminal transports like mosh and Eternal Terminal cannot. See [Remote attach over SSH](/docs/persistence-remote/#custom-transport) for the full contract and roaming guidance.
...
:::note
herdr-mx renders the expanded sidebar from its own `lines` segment layout (editable in the in-app sidebar settings), not from `rows`. The `rows` settings below are accepted, kept in sync with upstream herdr, and served over the socket API, but they do not change the mx sidebar yet. Pane and workspace metadata reporting works fully. See DIVERGENCE.md.
:::
...
`hide_nested_remote_panes` controls whether the multi-remote sidebar hides remote panes that are themselves running a `herdr --remote` client (a nested "mirror" view of another host). These panes are bounded and harmless but add clutter and keep a live client streaming on the remote. The default is `true`. Set it to `false` to show them. Plain local `herdr` panes are never hidden.
