...
- `{remote_command}` — the command Herdr runs on the remote (substituted within a token). **Required**: `args` must include a `{remote_command}` placeholder, otherwise the template is invalid (without it, Herdr would stream the remote payload — including the install binary — into a bare shell). An invalid template does not fail open to `ssh`: Herdr keeps the last valid transport it resolved this session, and only uses the built-in `ssh` transport if no valid custom transport has ever been resolved. The same keep-last-valid behavior applies when the config fails to parse or the `[remote]` section is invalid, so a transient bad edit can't silently redirect remote operations over `ssh`.
