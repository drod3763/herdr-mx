...
- `{remote_command}` — the command Herdr runs on the remote (substituted within a token). **Required**: `args` must include a `{remote_command}` placeholder, otherwise the template is rejected and Herdr falls back to the built-in `ssh` transport. Without it, Herdr would stream the remote payload (including the install binary) into a bare shell.
