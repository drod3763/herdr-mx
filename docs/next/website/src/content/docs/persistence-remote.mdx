...
By default, `herdr --remote` runs the bridge through a temporary SSH config that includes your SSH config first, then adds fallback keepalive settings. Existing user keepalive settings win. Set `[remote].manage_ssh_config = false` to use plain `ssh` without Herdr's generated bridge config. herdr-mx does not use upstream's per-attach control socket for connection reuse; use `[remote.transport]` (for example `autossh`) for reconnecting bridges.
...
- `program` — the transport binary to run. **Required** and non-blank: a present `[remote.transport]` with a blank or omitted `program` is treated as an invalid custom transport (not as "use ssh"), so Herdr keeps the last valid transport it resolved this session, or fails closed if none — it never silently downgrades to `ssh`. Omit the whole `[remote.transport]` table to use built-in ssh.
