...
Remote attach uses your normal SSH configuration; a configurable `[remote.transport]` (for example `autossh`) covers reconnecting bridges. herdr-mx does not apply upstream's generated-ssh-config keepalive or connection reuse — see DIVERGENCE.md.
