            ...
            if not is_skipped_field(field_attrs):
                fields.append(
                    StructField(
                        ...
                        doc=" ".join(doc_lines),
                        flatten=is_flattened_field(field_attrs),
                    )
                )
        doc_lines = []
        field_attrs = []
...
def is_flattened_field(attrs: list[str]) -> bool:
    return any(attr.startswith("#[serde") and "flatten" in attr for attr in attrs)
...
def collect_entries(model: Model, struct_name: str = ROOT_STRUCT, prefix: str = "") -> list[dict]:
    ...
    for struct_field in model.structs[struct_name]:
        ...
        # Path-qualified types (crate::config::sidebar::Foo) resolve by bare name.
        ...
        if inner in model.structs:
            ...
            # #[serde(flatten)] hoists the child struct's keys into the parent table.
            child_prefix = prefix if struct_field.flatten else f"{dotted}."
            entries.extend(collect_entries(model, inner, child_prefix))
        else:
